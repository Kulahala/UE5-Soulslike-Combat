...
private:
	...
	// 所有角色装备该武器时唯一使用的默认 Socket。普通武器默认右手；Bow 基类默认左手。
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Equip", meta = (AllowPrivateAccess = "true", ToolTip = "所有角色装备该武器时使用的骨骼 Socket。普通武器默认右手；Bow 基类默认左手。"))
	FName DefaultEquipSocketName = FName(TEXT("RightHandSocket"));
