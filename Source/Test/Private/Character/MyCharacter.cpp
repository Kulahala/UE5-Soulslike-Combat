...
/**
 * 响应玩家交互输入（如按下 E 键）。
 * 流程：前置状态校验 -> 重新仲裁目标 -> 接口合法性校验 (CanInteract) -> 触发交互 (Interact) -> 交互后二次仲裁刷新。
 */
...
/**
 * 玩家世界主动交互的前提守卫：
 * 必须不在火堆服务保护下、处于空闲常态 (EAS_UnOccupied)、存活且未处于空中下落状态。
 */
...
/**
 * 注册可交互候选物体（通常由掉落物/火堆的 OnSphereOverlap 触发）。
 * 采用弱指针池存储，注册后立即触发仲裁选出最优目标并刷新 UI。
 */
...
/**
 * 注销可交互候选物体（通常由 EndOverlap 或物体销毁触发）。
 * 从弱指针池中清理，并重新仲裁（若池空则触发 UI 隐藏）。
 */
 ...
 * 最优可交互目标仲裁中枢：
 * 1. 过滤清理失效弱指针；
 * 2. 筛选满足 CanInteract 条件的候选者；
 * 3. 按照【优先级 (Priority) > 水平距离平方 (DistanceSquared)】双规则仲裁出 BestCandidate；
 * 4. 防抖处理：目标无变化则直接返回，发生改变时更新 CurrentInteractable 并触发 UI 刷新。
 */
...
/**
 * 交互提示 UI 状态派发：
 * 若当前存在合法仲裁目标，多态提取提示词 (GetInteractionPrompt) 并通过 PlayerController 挂载到屏幕；
 * 若无有效目标则通知控制器隐藏并从屏幕视口移除。
 */
