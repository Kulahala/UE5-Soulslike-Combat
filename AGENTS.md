...
- Reusable compiler errors, runtime failures, repeated warnings, and tool failures belong in server-memory MCP, not `plan.md`. Record the trigger, symptom, wrong approach, correct approach, and scope. If server-memory is unavailable, say so and provide the proposed memory entry in the closeout. Do not query memory during strict review unless a concrete recurring pattern appears.
