...
- `code-review-graph` is a local incremental review-context aid. It supplements CodeGraph and direct source/diff inspection; graph edges, risk scores, `test gap` labels, and token-savings estimates are never findings or evidence by themselves.
- For a strict or stage-end review, use it when the executable is available and include a bounded graph packet in the fresh Reviewer brief: `status`, then `detect-changes --brief --base <explicit-stage-baseline>` when the worktree diff is limited to the review scope, plus `impact --files <reviewed-files> --base <explicit-stage-baseline>`. Never rely on its default `HEAD~1` baseline: use `HEAD` for an active uncommitted stage, or the recorded merge-base for a committed branch. When unrelated WIP shares the worktree, skip the aggregate `detect-changes` result and use the file-scoped impact query with the filtered Git diff instead.
- The main agent passes an extracted graph packet, never raw `impact` JSON: baseline, tool summary, changed symbols, affected external files, and at most ten graph relations relevant to the unresolved risk. For a strict or stage-end review, the fresh Reviewer must independently run `status` and one project-prescribed, file-scoped `impact` query before returning; the parent packet is not a substitute. It reports only a compact evidence line and may make one additional targeted graph expansion only when that evidence exposes a concrete dependency.
- Refresh with `update --skip-flows` only when relevant source edits are not indexed. That update changes ignored `.code-review-graph/` cache data, not gameplay source. Do not enable `watch`, `daemon`, `embed`, cloud embedding providers, `install`, or `uninstall` without an approved tooling task.
- The Reviewer begins with its required focused CodeGraph query, uses the graph packet only to prioritize caller/callee and blast-radius reads, and verifies every material relation against CodeGraph or source before reporting it. It must not widen scope, invent automated-test obligations, or turn a graph `test gap` into a finding without concrete code or validation evidence.
- Keep the existing Claude Code `SessionStart` status hook and post-`Edit|Write` incremental-update hook enabled as advisory, non-blocking cache maintenance. They do not run for Codex or other writers, so the main agent refreshes the graph explicitly before an independent review when needed.
