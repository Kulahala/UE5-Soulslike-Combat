...
- Runtime module: `Test`
- Local editor plugins in this checkout: `SmartBPCreator`, `UnrealBridge` (`UnrealBridge` is currently disabled in `Test.uproject`)
- Targets: `TestEditor` (Editor), `SoulslikeCombat` (Game)
- Build.cs dependencies: `Core`, `CoreUObject`, `Engine`, `InputCore`, `EnhancedInput`, `AnimGraphRuntime`, `Niagara`, `GeometryCollectionEngine`, `PCG`, `UMG`, `AIModule`, `NavigationSystem`, `Slate`, `SlateCore`, `MotionWarping`（其中 `Slate`、`NavigationSystem` 为 Private，其余为 Public）

<a name="game-flow-foundation"></a>
## Game Flow Foundation

- `UTestSaveGame` owns durable single-slot data. Save v3 strictly separates a valid writable record (`IsPersistable()`) from a usable respawn anchor (`HasRespawnAnchor()`): the latter additionally requires a non-empty `LastCheckpointId` contained in `ActivatedCheckpointIds`. `IsCompatible()` requires the exact current version, so v2 records are intentionally rejected rather than migrated.
- `USoulslikeGameInstance` owns slot I/O and carries the target gameplay map plus pending checkpoint ID across level transitions. `StartNewGame()` writes a candidate v3 record before replacing the current in-memory session and restores the previous session/map context when the write fails; callers must handle its `false` result rather than leave a menu transition in progress.
...
- `USoulslikeGameInstance` is the only authority that mutates durable item arrays. `AddOwnedItemInstance()`, `GrantAmmoReserve()`, `ConsumeLoadedAmmo()`, `SetEquippedItemSlot()`, the checkpoint-refill transaction, and fixed-world item/ammo claims write through `SaveNow()`. A checkpoint refill snapshots and rolls back `MapName`, `LastCheckpointId`, `ActivatedCheckpointIds`, `ItemInstances`, and `LoadedAmmoContainers` together; it prepares no gameplay transition until that write succeeds. Fixed weapon/shield claims snapshot `ItemInstances`, `ClaimedRewardIds`, and `EquippedSlots`; an ammo-bundle claim snapshots its Reserve stack mutation and `ClaimedRewardIds` in the same write, then rolls both back on failure. Development failure injection is separately scoped to fixed claims, loaded-ammo consumption, or a rest that actually transfers ammo. Components never access mutable `CurrentSaveGame` state directly, then update their local cache only after GameInstance success.
- `Aitem` owns common world-pickup trigger policy, eligibility, re-entrancy and success-only finalization. `Interact` registers the existing `E` candidate and resolves only while `AMyCharacter::CanInteractWithWorld()` is legal; `AutoOverlap` never becomes an `E` candidate and resolves after the item has reached `EIS_Dropped` whenever the player is alive and outside Bonfire service protection, including attack, dodge and falling. A continuous AutoOverlap attempts at most once; a failed grant leaves the Actor visible and retryable only after the player fully leaves and re-enters the Sphere. Multiple player primitive overlaps are treated as one actor-level overlap. `AWeapon`, `AShield`, `AAmmoPickup`, and `ATreasure` implement only their narrow grant hook; the base performs sound, candidate cleanup, collision shutdown and destroy only after that hook succeeds.
- Fixed-world `AWeapon`, `AShield`, and narrow `AAmmoPickup` retain their persistent-claim path: each requires a non-empty, map-unique `PersistentId` and `ItemDefinitionId`; `AAmmoPickup` additionally accepts only an ammo-container Definition with a positive `PickupQuantity`. On map load a claimed reward destroys its world Actor. Any transient player or enemy equipment must set its Owner, and where applicable its Instigator, in `FActorSpawnParameters` before `BeginPlay`; that deliberately bypasses fixed-world-pickup validation. Invalid, duplicate, unknown, failed writes, or failed required candidate materialization leave the Actor visible and interactable with a warning.
- `ATreasure` owns only Gold value and presentation. Its grant calls `AMyCharacter::TryAddGold()`, which updates `UAttributeComponent` and the HUD only after `USoulslikeGameInstance::TryAddGold()` has committed one `SaveNow()` write; failed or injected saves restore the prior durable Gold and leave the Treasure unchanged. `BP_Treasure_Base` hides its inherited static Mesh and uses its inherited `Effect` component with `/Game/Effects/Niagara/NS_Soul` as the shared Gold light-orb presentation. `GoldDebugFailNextClaimSave` is a development-only one-shot failure injection on `ACharacterController` for that transaction.
- `AEnemy` owns author-authored transient death rewards through `DeathTreasureClass` and `DeathGoldValue`. `Die()` attempts exactly one deferred `ATreasure` spawn at the death transform, writes Gold and `Interact` policy before `FinishSpawningActor()`, and never attaches the reward to the corpse. The resulting enemy Gold drop survives corpse lifespan cleanup but has no SaveGame identity, so existing Rest, death reload and map reload naturally remove unclaimed drops. `BP_Treasure_Base` also uses `Interact` as its CDO policy, so destructible and enemy Gold become `E` candidates only after reaching `EIS_Dropped`; the enemy still writes the instance policy explicitly to prevent child-CDO drift.
- A non-Encounter `AEnemy` instance can instead author a stable `DefeatId` and an optional aligned `RewardId` / `RewardPickupClass` / `RewardItemDefinitionId` / `RewardQuantity` tuple. `USoulslikeGameInstance` is the only owner of `DefeatedOneTimeEnemyIds` and `PendingOneTimeRewardIds`: a successful `TryCommitOneTimeEnemyDefeat()` adds both states in one `SaveNow()` transaction, while a save failure rolls both back and leaves the current death transient. `OneTimeEnemyDebugFailNextDefeatSave` arms only that next commit; `ItemDebugDump` reports the saved Defeated and Pending sets.
...
- `AWeapon -> ABowBase (abstract) -> ABow` is the Bow class hierarchy. `ABowBase` owns the shared physical Bow runtime: default left-hand attachment, collision-free `BowSkeletalVisual`, fixed `BowArrowSocket` contract, transient Bow presentation state, prepared-projectile nocking, and the only launch-transform query. With a valid physical Profile, it renders the Skeletal Bow and hides the inherited static `Mesh`; that static Mesh remains only as the attachment and `BoxTrace` compatibility anchor. `ABow` remains the player MainHand ranged-delivery configuration owner for the arrow Definition ID, projectile class and immutable delivery defaults, aim movement multiplier, optional `AimRaiseMontage`, required `DrawMontage` / `ReleaseMontage` / `LoadMontage`, audio, and the player-only loaded-arrow fallback visual. `BP_ErikaBow` derives directly from `ABowBase`; enemy attack selection, LOS policy and Projectile release remain owned by the Enemy DataAsset/HFSM path. `AimMoveSpeedMultiplier` is relative to `AMyCharacter::WalkSpeed` and defaults to `1.0`, so default Bow Aim uses the same base speed as player walking. `ABow` does not own inventory state, firing input, SaveGame writes, HUD, or world pickups.
- `UBowPhysicalProfileDataAsset` is the only authoring source for a Bow's Skeletal Mesh, Bow AnimInstance class, Skeletal-visual relative transform, nocked-arrow Mesh and nocked-arrow pivot correction. It does not own player ammo, projectile delivery, damage, character Montages, HUD or enemy timing. The concrete profiles are deliberately independent where the authored Bow assets differ: `DA_BowPhysical_Archer` drives DarkKnight's `SK_Bow` / `ABP_DarkKnightBow`, while `DA_BowPhysical_Erika` drives Erika's smaller `SKM_ErikaBow` / `ABP_ErikaBow`. `BP_ErikaArcher` directly references that enemy Bow class. They share the Profile type and the fixed Mesh-only `BowArrowSocket` contract, not a forced common Mesh, transform or AnimBP.
- `AWeapon::DefaultEquipSocketName` is the single runtime MainHand attachment contract for players and enemies. Ordinary weapons default to `RightHandSocket`; `ABowBase` changes that CDO default to `LeftHandSocket`. The property rename from `PlayerEquipSocketName` is preserved through a Core Redirect. `AMyCharacter` MainHand materialization and `AEnemy::WeaponInit()` both read the weapon default directly; no `AEnemy` character-level Socket override or second resolution path exists. A missing Socket or attachment failure rejects the candidate before a player equipment-slot write, or destroys the enemy candidate without retaining an invalid `EquippedWeapon`. DarkKnight authors that existing `LeftHandSocket` as the common left-palm weapon grip; `LeftHandShieldSocket` is its dedicated `hand_l` shield-back socket, and `AShield` / `BP_Shield` use it through the separate OffHand contract. Erika's `SK_ErikaArcher` supplies its own `LeftHandSocket` grip for `BP_ErikaBow`; that character-local grip remains outside the Bow Profile. `AShield` cannot enter `AEnemy::WeaponClass`.
...
| `EPlayerActionType` | `None`, `Attack`, `Dodge`, `Block`, `Parry`, `Potion`, `HitReact`, `Death`, `RangedAim`, `RangedRelease` | `AMyCharacter` current/final action identity; private intent resolver and resolved-action executor |
...
├── AEncounterController (placed encounter lifecycle, preplaced/initial-batch activation, and runtime boundary segments)
├── AEncounterSpawnPoint (initial-batch candidate placement only)
...
- `ProjectileDebugFire` and `ProjectileDebugFireSelf <Enemy|Player>` are non-Shipping C++ validation commands only. They do not replace the player Bow path, map Actor, visual asset, or ranged AI. The normal debug command derives direction from control rotation but starts outside the controlled capsule; the real player Bow path instead resolves its origin from `ABowBase::TryGetLaunchTransform()` and the fixed `BowArrowSocket`.
...
- **意图先决策，动作后执行**：`OnAttackInputPressed/Released/Canceled()`、Block Press/Release、Dodge、Parry 与 Potion 都先进入私有 `ResolvePlayerActionIntent(...)`。它以当前动作、Held/Committed 阶段和无副作用 preflight 决定唯一结果：`StartNow`、`BufferOnce`、`Reject` 或 `EndHeld`。`TryStartAction(EPlayerActionType)` 只执行已解析的目标动作，不能再做数值 Priority 或输入竞争判断；`HitReact`、Guard Break、Death、Bonfire、装备 teardown 与 EndPlay 保持外部硬中断，不通过 player resolver 启动。
...
- **配置入口**：主角 Blueprint 只配置 `PlayerProfile`；`AMyCharacter` 通过 `PlayerProfile->AttackConfig` 读取主角攻击配置，通过 `PlayerProfile->ActionConfig` 读取 Dodge / Block / Parry / Potion Montage，通过 `PlayerProfile->ReactionConfig` 读取受击和死亡 Montage。主角攻击 Montage 只走 `UAttackConfigDataAsset`；敌人攻击 Montage 只走 `UEnemyAttackConfigDataAsset`。旧 `ABaseCharacter::AttackMontage` 字段已删除。
...
- **ActionConfig 范围**：`UPlayerActionConfigDataAsset` 定义主角专属动作配置：`Dodge.Montage/StaminaCost`、`Block.Montage/BlockRaiseSection/StaminaRegenMultiplier`、`GuardBreak.Montage`、`Parry.Montage`、`Potion.Montage/HealPercent/Cooldown/FallbackHealSound`。它不定义或暴露数值 Priority 或输入仲裁语义。`Attack` montage/连招仍归 `PlayerProfile->AttackConfig`；主角 `HitReact` / `Death` montage 配置归 `PlayerProfile->ReactionConfig`；敌人当前归 `AEnemy::HitReactionConfig`。
