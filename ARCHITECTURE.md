...
- `ABow : AWeapon` is the MainHand ranged delivery configuration owner. It provides the arrow Definition ID, projectile class and immutable delivery defaults, authored projectile spawn point, aim movement multiplier, shot cooldown, and optional future presentation references. `AimMoveSpeedMultiplier` is relative to `AMyCharacter::WalkSpeed` and defaults to `1.0`, so default bow aim uses the same base speed as player walking. It does not own inventory state, firing input, SaveGame writes, HUD, or world pickups.
- `AWeapon::PlayerEquipSocketName` is the current player MainHand materialization attachment contract. It defaults to `RightHandSocket`; the `ABow` CDO changes it to `LeftHandSocket`. A missing socket or attachment failure rejects the runtime candidate before the equipment-slot write. Enemy weapon attachment remains on its existing per-enemy path until the future shared-Bow migration is explicitly adopted.
...
- `UPlayerHUDWidget` owns a presentation-only, viewport-centered hollow-cross aim reticle. `AMyCharacter` pushes the effective `IsBowAiming()` result only after aim entry, unified cancellation, and HUD creation; `SetAimReticleVisible()` invalidates Paint only on a value change. `NativePaint()` draws above the existing UMG, damage vignette and player debug text. The HUD does not infer or mutate aim state, projectile trajectory, target selection, ammo, damage, input, or SaveGame state.
...
- `TODO-04B-B` owns the visible player Bow Mesh, player arrow visual, proper Bow locomotion BlendSpace/state and Draw/AimHold/Release authoring. The B2 runtime contract deliberately does not use a Montage-only substitute for those persistent locomotion poses.
...
│   │   └── ABow (player MainHand ranged delivery configuration)
