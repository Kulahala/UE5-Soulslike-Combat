...
- `UItemOwnershipComponent` is the spawned Pawn's validated cache. It restores only records whose definition, instance ID, quantity, upgrade level, and slot compatibility are valid; malformed, duplicate, unknown, or incompatible records emit warnings and are ignored for that Pawn without rewriting the original save. For non-equippable definitions it exposes total-quantity query, grant, and consume operations plus `OnOwnedItemQuantityChanged`; the delegate is a presentation notification, not a mutable SaveGame reference.
- `USoulslikeGameInstance` is the only authority that mutates durable item arrays. `AddOwnedItemInstance()`, `ConsumeOwnedItemQuantity()`, `SetEquippedItemSlot()`, and the shared fixed-world-claim transaction write through `SaveNow()`. A claim with an optional empty MainHand/OffHand request snapshots and rolls back `ItemInstances`, `ClaimedRewardIds`, and `EquippedSlots` together; C1's one-shot debug save failure is consumed only by this fixed-world transaction, while the quantity failure injection is scoped to the next valid quantity consume. Components never access mutable `CurrentSaveGame` state directly, then update their local cache only after GameInstance success.
...
- On a valid release, `AMyCharacter` creates a collision-disabled prepared projectile, then asks `UItemOwnershipComponent` to consume one non-equippable arrow through `USoulslikeGameInstance`. Only a successful write updates the Pawn cache and activates the projectile. The current pool is the sum of `FTestItemInstanceRecord.Quantity`; it deliberately has no loaded-arrow container, reserve stack, rest refill, field reload, or backpack UI until `TODO-04B-A2`.
- `ItemDebugGrantQuantity <DefinitionId> <Quantity>` and `BowDebugFailNextAmmoConsumeSave` are non-Shipping validation-only commands. They exercise the quantity path but do not create a player inventory surface or a second gameplay firing route.
...
- `SpawnConfiguredProjectile()` uses deferred spawn and sets Owner/Instigator before `BeginPlay` for immediate delivery. `SpawnPreparedProjectile()` uses the same validated snapshot but leaves collision, movement, and lifetime inactive until its caller explicitly calls `ActivateConfiguredProjectile()`. `BeginPlay` ignores the attacker/instigator for movement; `OnProjectileStopped()` closes collision, guards against a second resolution, then destroys the Actor after its one outcome; `EndPlay` removes the stop delegate.
