...
- `BP_DarkKnightBow` is the player presentation child of `ABow`: it authors the shared physical Bow Mesh, `LoadedArrowAnchor`, collision-free `LoadedArrowVisual`, `ProjectileSpawnPoint`, Draw/Release Montage and shot/empty audio. `LoadedArrowVisual` exists only while the player is in a valid Aim state with Loaded ammo and no active release presentation. `USlashAnimInstance::bIsBowAiming` reads only `AMyCharacter::IsBowAiming()`; the authored non-Aim and Aim Bow locomotion BlendSpaces are therefore presentation consumers, not a second action-state source.
...
- `UPlayerHUDWidget` owns a presentation-only, viewport-centered hollow-cross aim reticle plus optional `Text_ArrowCount`. `AMyCharacter` pushes the effective `IsBowAiming()` result and `Loaded / Capacity` after HUD creation, ownership restoration, Bow materialization, Rest, successful ammo changes and lifecycle cleanup. The HUD does not infer or mutate aim state, projectile trajectory, target selection, ammo, damage, input, or SaveGame state.
- On a valid release, `AMyCharacter` line-traces `ECC_Visibility` from the active camera to determine an aim point, then launches from `BP_DarkKnightBow.ProjectileSpawnPoint` toward that point. The Bow's authored nocked-arrow visual and spawn point therefore share one physical source while the reticle remains the aim authority. It creates a collision-disabled prepared projectile, then must successfully start the configured `ReleaseMontage` before asking `UItemOwnershipComponent` to consume one loaded arrow through `USoulslikeGameInstance`; only a successful consume performs `CommitPreparedLaunch()` and plays the shot sound. A missing AnimInstance/Montage, failed `Montage_Play()`, or physically still-playing prior Release Montage rejects before consumption, Commit or sound. The Montage lifecycle, including cancellation blend-out reported by `Montage_IsPlaying()`, is the sole re-fire gate: there is no numeric `ShotCooldown`. A durable consume failure destroys the prepared candidate, stops only the active Release Montage with the short blend-out, restores the nocked-arrow visual and preserves `EAS_Aiming`. Reserve remains in `FTestItemInstanceRecord.Quantity`; only a real bonfire Rest transfers reserve into `FTestAmmoContainerRecord` up to the authored capacity. No field reload, backpack UI, AnimNotify-driven fire timing, or second gameplay firing route exists.
...
- `EBowPresentationState` (`Relaxed`, `Aiming`, `Releasing`) is transient Bow-owned presentation state, not an `EActionState` or a SaveGame value. `AMyCharacter` writes it only after existing Aim/Release presentation gates succeed, restores it after release abort/end, and sets `Relaxed` before unified cancellation stops Montages. `UBowAnimInstance` reads only its owning `ABow`; `ABP_DarkKnightBow` consumes that value for its `Relaxed -> EnterAim -> AimHold -> Release/Cancel` state machine. Neither the Bow AnimBP nor an AnimNotify can consume ammo, spawn/commit a projectile, change input or recover player gameplay state.
