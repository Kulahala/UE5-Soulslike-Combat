...
- **Combat Local HFSM**: `EES_Combating` 拥有私有 `AEnemy::EEnemyCombatSubState` 枚举（`None`, `Orienting`, `AttackReadyPressing`, `CoordinatedWaiting`, `CooldownSpacing`, `RangedEscape`），显式化原先隐藏在 `OnCombating()` 中的战斗子状态。
  ...
  - **子状态语义**：
    ...
    - `RangedEscape` — 仅纯 Projectile 攻击配置在极近距离使用的私有高速逃离；它不是新的外层 `EEnemyState`，不会暴露给 AnimBP 或近战敌人
  ...
  - **Pure Projectile Escape**：仅当攻击配置至少有一条可选 Projectile 且不存在可选 Melee 条目时，`RangedEscape` 才能在其作者化 Enter/Exit 滞后范围内抢占普通战斗。其固定优先级是：已承诺的 `EES_Attacking` 蒙太奇，随后 `RangedEscape`，随后 pending AttackIntent，最后普通 MovementIntent；冷却和攻击协调只是 AttackIntent 的合法性 gate。一次 Escape 使用带独立 `FAIRequestID` 的固定导航腿，不随目标移动反复取消；仅真实导航失败或未抵达请求点时退化为 BackDiag，并在状态离开、受击、破防、死亡、Dormant 与 `EndPlay()` 清理瞬态。
...
- **Attack Configuration**: 敌人攻击行为由 `UEnemyAttackConfigDataAsset` 驱动，条目描述 montage / section、post-attack cooldown、`MinDistance` / `MaxDistance`、weight、damage multiplier、block-stamina multiplier、是否不可弹反、可选 Motion Warping 配置。缺少 DataAsset 时不会攻击，并输出配置警告；不再保留旧 `AttackMontage + Attack1` 硬编码回退。配置校验区分 DataAsset 自身校验与 `AEnemy` 边界校验（`CombatAttackMaxRadius`）。
  ...
  - **No New Enemy Class**: `AEnemy` remains the only C++ enemy base. D-A provides a non-Shipping `EnemyRangedDebugProbe [ReleaseDelay]` that exercises the same range, LOS, spacing, state and release guards with a temporary native Enemy/Controller pair; it does not create a map Actor, asset, persistence state or formal archer presentation. `TODO-04D-B` owns the `BP_ErikaArcher` content layer, Montage Release Notify placement, socket, visual bow and TestMap authoring.
  ...
  - **Distance Contract**: 起手执行距离上限为 `Min(Entry.MaxDistance, CombatAttackMaxRadius)`。近战仍以 `CombatAttackMaxRadius <= CombatPreferredMinRadius <= CombatPreferredMaxRadius < CombatingRadius` 维持冷却拉扯；纯 Projectile 配置允许其安全横移环位于实际出手窗口内，并额外由 Escape Enter/Exit 滞后与作者校验保证不和普通 Retreat 重叠。
  ...
  - **Near-Range Boundary**: Melee entries keep the original `MinDistance` + retry-block behavior. Projectile entries are the deliberate exception: when too close they retain their ranged intent and move back into their authored distance ring before trying to release again. A pure Projectile profile can additionally preempt that intent with `RangedEscape` below its stricter emergency threshold; it resumes normal range tactics only after reaching the separate Exit threshold.
