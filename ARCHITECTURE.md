...
- `TODO-03A` established data-only ownership. `TODO-03B-A` converts the two fixed world pickups into owned records and removes only successfully claimed world Actors; it does not change equipment slots, attach equipment, or change combat state. The former direct pickup attachment helpers remain inactive from world interaction and are reserved for `TODO-03B-B`, which will own checkpoint-time loadout selection plus materialization/restoration.
