...
- `Aitem` owns presentation-side fixed-world-pickup validation. Only `AWeapon` and `AShield` opt into the persistent-claim path: an unowned instance requires non-empty, map-unique `PersistentId` and `ItemDefinitionId`; on map load it destroys itself only when the reward ID is already saved as claimed. On `E`, successful GameInstance persistence precedes collision cleanup and Actor destruction. Invalid, duplicate, unknown, or failed writes leave the Actor visible and non-claimable or claimable as appropriate with a warning. `ATreasure` remains a separate overlap-driven Gold pickup.
...
- `TODO-03A` established data-only ownership. `TODO-03B-A` converts the two fixed world pickups into owned records and removes only successfully claimed world Actors; it does not change equipment slots, attach equipment, or change combat state. `TODO-03B-B1` establishes the activation/rest/menu boundary only. The former direct pickup attachment helpers remain inactive from world interaction and are reserved for `TODO-03B-B2`, which will own checkpoint-time loadout selection plus materialization/restoration.
