...
- [x] **TODO-03C0: Enemy Gold Drop And Pickup v1**
  Delivered the first transient Soulslike death-reward loop. A configured `AEnemy::Die()` deferred-spawns exactly one independent `ATreasure`, writes its Gold and `Interact` policy before `BeginPlay`, and leaves it after corpse cleanup; unclaimed enemy drops remain session-local and disappear through existing world reloads. `Aitem` now owns `Interact / AutoOverlap` policy, actor-level continuous-overlap retry control, common re-entrancy protection and success-only finalization, while Weapon, Shield, Ammo and Treasure keep narrow grant hooks. Gold now follows the single rollback-safe `ATreasure -> AMyCharacter::TryAddGold -> USoulslikeGameInstance::TryAddGold -> SaveNow` path, with a one-shot injected-save-failure check. `BP_Treasure_Base` now presents Gold as an `NS_Soul` light orb and uses `Interact` for both destructible and enemy Gold; Paladin and Erika still author one `10`-Gold reward each. The generic `AutoOverlap` policy remains available only when explicitly authored on another pickup. User manually compiled and PIE-validated enemy drops, `E` priority, corpse independence, the generic AutoOverlap path during attack/dodge/fall, failed-save retry behavior and reload cleanup. Main review plus independent adversarial review found no blocking defect.

- [x] **TODO-03C: One-Time Enemy Fixed Drops v1**
  Delivered the first special-elite persistence contract independent of `AEncounterController`. Strict SaveGame v3 records `DefeatedOneTimeEnemyIds` and `PendingOneTimeRewardIds`; a configured non-Encounter `AEnemy` writes defeat plus optional fixed reward atomically, keeps its normal transient Gold drop, and restores only an unclaimed pending reward at its authored initial transform after Rest, death reload or Continue. The first TestMap Paladin fixture awards the existing `BP_DarkKnightArrowBundle` as `Item_DarkKnightArrow x20`. Item/ammo claim success writes `ClaimedRewardIds` and removes Pending in the same save; failed defeat or claim writes preserve the appropriate retry/reload behavior. Invalid reward Definition or quantity is rejected before persistence, pending rewards cannot bypass the base item transaction through `ATreasure`, and failed initial deferred completion destroys the staged Actor while reload reconstruction remains authoritative. Encounter ownership rejects a one-time enemy. User manually compiled and PIE-validated the normal flow, reload restoration, invalid authoring fallback, Treasure-class routing, and both failure injections; normal review plus fresh delta adversarial review found no blocking defect. Completed in this commit.
...
- [ ] **TODO-06A: First Level Route And Integration v1**
  Deferred until the user authors the dungeon environment, positions the fog gate and Boss at its endpoint, and is ready to assemble the completed runtime features into one playable route. Block out and author the critical route: start checkpoint, two normal combat spaces, one elite space, one optional shortcut or previously proven one-time elite-reward branch, and the final Boss approach. This stage absorbs the former `TODO-02B`: author the first permanent `AEncounterController` in a normal combat space with a stable `EncounterId`, closed Spline boundary and its then-current supported participant mode. Normal and elite enemies use the completed `TODO-03C0` `Aitem -> ATreasure` Gold-drop loop; the optional branch must reuse an existing fixed pickup, shortcut, or the completed `TODO-03C` one-time enemy contract rather than introduce another reward system. It does not itself invent completion rules; Boss completion belongs to `TODO-06B`, final completion UI belongs to `TODO-06C`, and full end-to-end regression belongs to `TODO-07B`.

### One-Time Enemy Rewards

`TODO-02C0` owns only transient spawn configuration and runtime participants. `TODO-02C1` owns only Controller-managed encounter persistence. Neither is a final catch-all migration for pickups, one-time enemies, equipment, shortcuts, rewards or Boss state. Each durable feature adds its own read/write path when its stable ID and reset contract are implemented, so that feature can be validated in the player loop that introduces it. `TODO-07B` is the final cross-system Demo regression route; it verifies completed persistence paths together and fixes regressions, but does not defer their original implementation.

- [ ] **TODO-06B: First Boss Encounter v1**
  Add one Boss fog-boundary presentation, configured first Boss as an `AEnemy` variant, fixed Gold plus Remnant/Emblem reward, and Boss completion persistence. Reuse `AEncounterController` and the completed `TODO-02C0` spawn contract with one fixed `Point` anchor and one Boss member by default; the fog boundary is Controller presentation, not a second spawning system. The first Boss must escalate through `Phase 1 -> Transition -> Phase 2` and include at least one authored complete attack-chain pattern: AI selects that pattern once rather than re-rolling every hit, so a shared prefix may branch as `M1 -> M2 -> M3 -> M4` or `M1 -> M2 -> M3 -> M5`. The later 06B plan must choose from the real animation assets whether those nodes use explicit separate-Montage handoffs or Sections inside one Pattern Montage, while preserving pose continuity and interrupt cleanup. Do not introduce a Boss-specific C++ base unless the existing enemy and encounter boundaries demonstrably fail.
...
- **Pre-repair v3 malformed Pending rewards are not migrated.** New one-time defeat writes validate Definition and quantity before adding `DefeatedOneTimeEnemyIds` / `PendingOneTimeRewardIds`, but an invalid Pending already written by a pre-repair v3 development build, or a Definition later removed from the player catalog, can still be reconstructed by `AEnemy::HandleRestoredOneTimeDefeat()` and remain unclaimable after reload. **Player impact:** the special enemy stays defeated while its light orb repeatedly returns but cannot be collected; no duplicate item, Gold or new persistent state is created. **Resolution condition and owner:** before supporting existing v3 development saves across Definition-catalog removal or shipping a content update that removes a claimable Definition, add and validate an explicit one-time-reward recovery/migration transaction in that save-compatibility stage.

- **Non-ammo stackable consumption has no generic runtime route yet.** The obsolete raw-`DefinitionId` consumer is removed; current item ownership only exposes validated equipment, fixed world-item, and ammo transactions. **Resolution condition:** before the first non-ammo stackable consumable, drop, or backpack route, introduce a Pawn-validated instance-selection consumption transaction and cover malformed-record and shifted-index cases in that owning stage.
