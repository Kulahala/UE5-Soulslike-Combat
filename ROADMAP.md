...
- [ ] **TODO-03C: One-Time Enemy Fixed Drops v1**
  Prerequisite: `TODO-02C1` has proven a rollback-safe durable clear marker transaction. Add an optional, author-configured one-time enemy contract independent of `AEncounterController`: a stable `DefeatId` prevents only that special enemy from returning after Rest, death reload or Continue, and an optional stable `RewardId` represents a guaranteed fixed item drop. On death, the fixed reward becomes durable pending state before its interactable light orb is shown; a successful item transaction marks it claimed. An unclaimed fixed reward recreates from its authored fallback anchor after reload. First proof uses one non-Encounter world elite with an arrow bundle. A `DefeatId` enemy is not an `AEncounterController` participant in this first version, so death before a group clear cannot create an unowned half-cleared Encounter; a future integration needs its own explicit partial-clear policy. Ordinary enemies and chance drops remain transient, roll only at death, and disappear on world reload when unclaimed. Do not add a generic drop table, random affixes, a universal loot component, or migrate every enemy to one-time persistence.
