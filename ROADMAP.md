...
- [ ] **TODO-06A: First Level Route And Integration v1**
  Deferred until the user authors the dungeon environment, positions the fog gate and Boss at its endpoint, and is ready to assemble the completed runtime features into one playable route. Block out and author the critical route: start checkpoint, two normal combat spaces, one elite space, one optional shortcut/fixed-reward branch and the final Boss approach. This stage absorbs the former `TODO-02B`: author the first permanent `AEncounterController` in a normal combat space with a stable `EncounterId`, closed Spline boundary and its then-current supported participant mode. Normal and elite enemies use the completed `TODO-03C0` `Aitem -> ATreasure` Gold-drop loop; the optional branch must reuse an existing fixed pickup or shortcut rather than introduce another reward system. It does not itself invent completion rules; Boss completion belongs to `TODO-06B`, final completion UI belongs to `TODO-06C`, and full end-to-end regression belongs to `TODO-07B`.
