...
- [ ] **TODO-04B-C: Player Bow Charge And Confirmed Hit Feedback v1**
  Prerequisite: `TODO-04B-B3` has validated DarkKnight Bow aim orientation, Draw/Aim/Release presentation, and player-owned visual arrow. Add only player-owned charge and feedback: RMB remains held aim, while LMB press starts Draw/charge and LMB release fires; a real held-charge value owns explicit cancellation/release rules, reticle spread-to-focus, and a small local Camera FOV blend that restores on release/cancel without creating target lock or camera reframing. First evaluate the retargeted In-Place `Bow_InPlace_Aim_Pull` as the charge-start presentation, `Bow_InPlace_Idle_Aim_01` / Aim locomotion as the hold, and `Bow_InPlace_Shoot_2_Aim` as the release; final timing follows actual Skeleton-compatible asset preview, not source names alone. Add a short center hit marker only after a confirmed hostile projectile result. Enemy archers retain their existing direct release and receive no player HUD, charge UI, FOV blend, or hit marker. Keep the first player version zero-gravity/direct-fire; a ballistic arc requires a later dedicated aiming, obstruction and projectile-tuning decision rather than an incidental UI change.

- [ ] **TODO-04B-D: Bow Profile Data Assets v1**
  Prerequisite: `TODO-04B-B3` has passed and a second player Bow, an NPC weapon-drop route, or a genuinely shared Bow/projectile configuration is selected. Introduce narrowly scoped authored profiles rather than forcing player input and enemy tactics into one asset: a shared Bow/projectile identity profile may own reusable projectile actor, ammo and physical/visual identity, while a player presentation profile owns DarkKnight-specific Draw/Release, locomotion and aim assets. The selected Bow / Item Definition resolves that player profile; `AMyCharacter` remains the action-state owner that starts, cancels and receives Montage-end callbacks, while `ABow` remains the runtime owner of loaded-ammo transactions, prepared-to-commit firing, visual-arrow components and spawn-point access. Do not move Bow Draw/Release into generic `UPlayerActionConfigDataAsset`, which is for character-wide actions and would become a weapon-specific catch-all. Keep `UEnemyAttackConfigDataAsset` responsible for AI range, cooldown, target policy, enemy Montage timing and tactical behavior. Migrate only proven duplicate configuration and retain one clear authoring entry per responsibility.
