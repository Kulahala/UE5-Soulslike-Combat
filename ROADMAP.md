...
- [ ] **TODO-04B-B2a: Shared Bow Identity And Attachment Defaults v1**
  Prerequisite: `TODO-04B-B2` is committed and its B2 runtime contract remains stable. Refactor only the shared physical Bow identity and attachment defaults into `AWeapon -> ABowBase -> ABow` (or a future player-only `APlayerBow`) plus `BP_ErikaBow`; `ABowBase` owns generic Bow presentation identity and the default attachment contract, while player ammo, Prepared/Commit launch, aiming and cooldown stay in the player Bow branch and enemy Projectile snapshot, LOS, Release and HFSM behavior stay in `FEnemyAttackEntry` / `AEnemy` / `ARangedEnemy`.

  Replace the current player-only attachment property through a backward-compatible reflected-property and Blueprint migration: ordinary weapons default to `RightHandSocket`, Bows default to `LeftHandSocket`, and `AEnemy::WeaponAttachSocketName = NAME_None` means use the weapon default while an explicit per-character override remains valid for Paladin, nonhuman skeletons and exceptional authoring. Do not blindly rename a serialized `UPROPERTY`; preserve or deliberately migrate existing Blueprint values, reparent `BP_ErikaBow` only after its compiled defaults are read back, and verify player Bow, Erika Bow, Paladin, fallback socket, explicit override, B2 behavior and unchanged enemy attack-data ownership. Future drops grant an `ItemDefinitionId`; they never hand the player an enemy's live `EquippedWeapon` Actor. This architecture stage adds no actual loot, world pickup, Mesh, Montage or player Bow presentation.

- [ ] **TODO-04B-B: Bow Presentation And World Pickup v1**
  Prerequisite: `TODO-04B-B2a` has passed, and a DarkKnight-compatible aim/release animation set plus shot/empty audio assets are selected. Reuse the shared Bow visual identity where it passes the B2a attachment contract, but retain player-only `ABow` gameplay configuration and attachment/spawn-point authorship. Author the visible player bow, visible arrow, persistent `TestMap` bow and 20-arrow bundle pickups, and permanent loaded-arrow HUD. Use the established left-hand Bow / right-hand draw contract; do not substitute sword animations as a final bow presentation. This follows the enemy archer/target-switching validation path.

- [ ] **TODO-04B-C: Player Bow Charge And Confirmed Hit Feedback v1**
  Prerequisite: `TODO-04B-B` has a validated DarkKnight bow, Draw/Aim/Release animation contract, and player-owned visual arrow. Add only player-owned charge and feedback: a real held-charge value with explicit cancellation/release rules, reticle spread-to-focus driven by that value, and a short center hit marker only after a confirmed hostile projectile result. Enemy archers retain their existing direct release and receive no player HUD, charge UI, or hit marker. Keep the first player version zero-gravity/direct-fire; a ballistic arc requires a later dedicated aiming, obstruction and projectile-tuning decision rather than an incidental UI change.
