...
- [ ] **TODO-04B-C: Player Bow Charge, Nocked Projectile Handoff And Confirmed Hit Feedback v1**
  Prerequisite: `TODO-04B-C0` has validated the Skeletal Bow, `BowArrowSocket` and nocked-arrow spatial contract. Add only player-owned charge and feedback: RMB remains held aim, while LMB press starts Draw/charge and LMB release requests fire; a real held-charge value owns explicit cancellation/release rules, reticle spread-to-focus, and a small local Camera FOV blend that restores on release/cancel without creating target lock or camera reframing. Reuse the existing prepared `AArrowProjectile : ACombatProjectile` as the one nocked candidate during held charge rather than introducing a second Arrow `UActorComponent` or putting collision on the Bow: spawn/configure it through the existing prepared route, attach its collision root to `BowArrowSocket` while collision and ProjectileMovement are inactive, and show its own ArrowVisual. Only a successful durable loaded-ammo consume detaches that same Actor with its current world transform and calls `CommitPreparedLaunch()`; it then owns its existing velocity, collision, hit and lifetime path without a second spawn. Charge cancellation, invalid prepare, release-presentation failure, save failure, hit, Guard Break, death, equipment change and teardown destroy the uncommitted candidate and restore the valid Aim-ready visual without consuming ammunition or leaving an attached Actor. The C0 static nocked visual is a temporary Aim-ready representation only: it must hide whenever the prepared Actor is attached, and may be removed only after the later chosen Aim-ready handoff passes every cancellation and persistence-failure path. First evaluate the retargeted In-Place `Bow_InPlace_Aim_Pull` as the player charge-start presentation and the Bow-side `Bow_Aim_Pull` as string tension, then use `Bow_InPlace_Idle_Aim_01` / Aim locomotion and the Bow AimHold as the hold; final timing follows actual Skeleton-compatible asset preview, not source names alone. Add a short center hit marker only after a confirmed hostile projectile result. Enemy archers retain their existing direct release and receive no player HUD, charge UI, FOV blend, nocked prepared Actor, or hit marker. Keep the first player version zero-gravity/direct-fire; a ballistic arc requires a later dedicated aiming, obstruction and projectile-tuning decision rather than an incidental UI change.

- [ ] **TODO-04B-D: Bow Profile Data Assets v1**
  Prerequisite: `TODO-04B-C` has passed and a second player Bow, an NPC weapon-drop route, or a genuinely shared Bow/projectile configuration is selected. Introduce narrowly scoped authored profiles rather than forcing player input and enemy tactics into one asset: a shared Bow/projectile identity profile may own reusable projectile actor, ammo and physical/visual identity, while a player presentation profile owns DarkKnight-specific character Draw/Release, locomotion, aim assets and the player-only Skeletal Bow visual contract (AnimBP, sequence set and nocked-arrow socket). The selected Bow / Item Definition resolves that player profile; `AMyCharacter` remains the action-state owner that starts, cancels and receives Montage-end callbacks, while `ABow` remains the runtime owner of loaded-ammo transactions, prepared-to-commit firing, visual-arrow components and spawn-point access. Do not move Bow Draw/Release into generic `UPlayerActionConfigDataAsset`, which is for character-wide actions and would become a weapon-specific catch-all. Keep `UEnemyAttackConfigDataAsset` responsible for AI range, cooldown, target policy, enemy Montage timing and tactical behavior. Migrate only proven duplicate configuration and retain one clear authoring entry per responsibility.
...
### Player Melee Authoring

- [ ] **TODO-05C: Player Combo Per-Entry Montage Handoff v1**
  Sequencing: schedule after `TODO-04B-C` closes the player Bow held-input route, before additional melee weapon families or a larger light-combo library makes the current shared Montage harder to migrate. Replace `UComboDataAsset`'s single `ComboMontage + SectionName` contract with one complete Montage and entry Section per `ComboChain` entry, while retaining each entry's damage, stamina, poise and Motion Warping data. Each module owns `Entry -> ComboWindow -> ComboBranchPoint -> Recovery -> End`; do not split one attack into separate wind-up, strike and recovery assets. For the current cumulative source clips, author non-destructive project-owned `B -> Recovery` and `C -> Recovery` ranges or equivalent entry Sections, without editing Marketplace sources. The DataAsset may reorder or replace compatible modules, including replacing only the third hit while leaving the first two unchanged, but animation pose continuity and the existing collision/Notify contract remain authoring requirements. `ComboWindow` must only buffer input; `ComboBranchPoint` performs an intentional old-to-new Montage handoff. The C++ path must distinguish that planned `bInterrupted` delegate from a real hit, dodge, Guard Break, death or cancellation so the old Montage cannot reset action state, stamina regeneration or combo progress. `Montage_JumpToSection` remains intra-Montage only. Validate a third-entry-only replacement, compatible order changes, natural no-input recovery, deliberate handoff, collision/sound/Motion-Warping continuity, and all interruption/teardown paths.
...
Status: the current three-hit cumulative source set now has a confirmed per-entry replacement and local recovery-tuning need. `TODO-05C` owns the migration; the live implementation remains the shared-Montage path until that stage is explicitly promoted into `plan.md`.
