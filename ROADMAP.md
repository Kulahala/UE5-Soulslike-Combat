...
- [ ] **TODO-04B-B3: Player Bow Aim Orientation And Grip Presentation v1**
  Prerequisite: `TODO-04B-B` has passed its functional Bow pickup, firing, HUD and basic locomotion validation. Complete only the remaining player-facing aim/grip gaps: add a DarkKnight-only `LeftHandBowSocket` whose palm-oriented transform is authored for the Bow grip, then override `BP_DarkKnightBow` to use it while preserving `ABowBase` / `BP_ErikaBow` on their existing `LeftHandSocket` contract. Align the Bow mesh first, then its child `LoadedArrowAnchor`, `LoadedArrowVisual` and `ProjectileSpawnPoint`; the visual arrow remains Bow-owned and must not attach directly to a hand socket. While `EAS_Aiming` without lock-on, use controller yaw for the character's horizontal facing so camera-relative movement becomes forward/strafe/back relative to the reticle; do not create a target lock, camera reframing, or a second rotation-state system, and restore the prior normal rotation mode on aim cancel while preserving existing lock-on priority. First evaluate the current non-additive `Bow_Idle_Aim_BlendSpace` as a Control-Rotation-driven upper-body replacement layer over Bow Aim locomotion. Its `HorizontalAngle` / `VerticalAngle` inputs must receive local aim yaw/pitch and its branch filter must start above the lower body. Only if that absolute-pose layer visibly fights locomotion or shoulders should the stage derive or author a Mesh Space additive Aim Offset from its center pose. Do not change player release/ammo transactions, enemy AI aiming/release, Erika sockets, or global Skeleton attachment defaults.
