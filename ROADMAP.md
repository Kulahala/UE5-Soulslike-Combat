...
- [ ] **TODO-04D-D: Ranged Enemy Specialization And Attack Cancellation v1**
  Prerequisite: `TODO-04D-C` is committed. Introduce `ARangedEnemy : AEnemy`, not a new `ABaseEnemy` / `AMeleeEnemy` migration. Keep `AEnemy` as the shared owner of perception plumbing, target validity, movement primitives, hit/death state, generic attack-entry execution and Projectile delivery; move the pure-ranged spacing/escape policy and its authoring defaults behind the derived class so `BP_ErikaArcher` no longer has to emulate an archetype through unrelated Blueprint overrides. Establish three speed roles: non-combat Patrol `220 / 200 cm/s` for Erika / Paladin, normal combat Retreat/BackDiag/Strafe `300 / 290`, and high Press/Chase/Escape `330 / 330`; native class defaults provide those baselines while Blueprints retain only intentional archetype exceptions. When a projectile attack has not yet released and LOS becomes blocked, cancel that active ranged Montage through a state-guarded shared cleanup path with no projectile or late Notify; prefer an authored Cancel/Recover section and allow a short `Montage_Stop` blend only as the no-asset fallback. Do not interrupt an already released projectile, melee attack, hit reaction or death montage. Future caster classes may inherit `ARangedEnemy` only when they share this tactical contract; create a separate class only for real runtime behavior, not for a different mesh or attack DataAsset.
...
- **A pre-release archer can visibly finish an empty Montage after LOS is lost.** `TryReleaseConfiguredProjectileAttack()` correctly rejects a blocked LOS and never creates an arrow, but the current `Draw -> AimHold -> Release` asset has no cancellation route, so a player stepping behind a wall may still see the release finish. This has no damage, persistence or state-corruption impact. **Resolution condition:** `TODO-04D-D` must add a state-guarded unreleased Projectile abort path, prefer an authored Cancel/Recover section, and use a bounded `Montage_Stop` blend only when no authored cancel is available; verify that a late Release Notify cannot fire and that an already released arrow is never recalled.
...
Recommendation: retain `AEnemy` as the shared enemy core and, after `TODO-04D-D`, use `ARangedEnemy : AEnemy` for a genuinely shared ranged tactical contract. Do not extract `ABaseEnemy` or add an `AMeleeEnemy` merely to make the hierarchy symmetric: the existing `AEnemy` already is the common runtime base, and Paladin currently has no melee-only behavior to share with a second concrete subclass. Attack entries remain DataAsset-authored; an archetype class owns behavior policy and safe default values, not an alternate attack database.
