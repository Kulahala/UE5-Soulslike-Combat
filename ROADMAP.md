...
- [ ] **TODO-05A1: Combat-Aware Sprint Stamina v1**
  Prerequisite: `TODO-05A` closeout confirms the current Exhausted and Guard Break contracts. Add one runtime-only player Combat Presence predicate with explicit enter sources: an enemy actively engages the player as its combat target, or the player deals/receives a valid hostile combat hit. Retain Combat Presence for a default `4 s` exit hysteresis after the final source clears. Only while that predicate is true may the existing Shift sprint debit stamina and reset stamina-regeneration cooldown; outside combat, Shift sprint keeps its current speed and hearing-noise behavior but costs no stamina. Do not use lock-on as a combat proxy, do not create an `EActionState`, do not persist Combat Presence, and do not alter attack, dodge, block, Guard Break, or ordinary `RunSpeed` rules. Validate target death/loss, encounter dormancy, player death/reload, PIE end, hit-only engagement without lock-on, and transition into combat while already sprinting.
