...
- [ ] **TODO-06A: First Level Route v1**
  Block out and author the critical route: start checkpoint, two normal combat spaces, one elite space, and one optional shortcut/fixed-reward branch. This stage absorbs the former `TODO-02B`: author the first permanent `AEncounterController` in a normal combat space with a stable `EncounterId`, closed Spline boundary, and pre-placed participants, then validate `Idle -> Active -> Cleared` in the real level. Normal and elite enemies use the completed `TODO-03C0` `Aitem -> ATreasure` Gold-drop loop; the optional branch must reuse an existing fixed pickup or shortcut rather than introduce another reward system. Use the completed encounter, equipment, ranged, and guard-break systems; do not add waves, encounter persistence, Boss content, a completion screen, or a new framework here.
