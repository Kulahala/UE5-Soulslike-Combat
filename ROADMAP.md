...
- [ ] **TODO-04B-B: Bow Presentation And World Pickup v1**
  Prerequisite: `TODO-04B-B1` is complete, and a DarkKnight-compatible aim/release animation set plus shot/empty audio assets are selected. Reuse of Erika's visual bow Mesh is allowed, but the player Bow actor must retain its own `ABow` gameplay configuration and attachment/spawn-point authorship. Current MainHand materialization uses the shared `RightHandSocket`; if the selected DarkKnight bow must attach to a different hand, add a narrow per-weapon attachment-socket contract instead of moving the global sword socket. Author the visible player bow, visible arrow, persistent `TestMap` bow and 20-arrow bundle pickups, and permanent loaded-arrow HUD. Do not substitute sword animations as a final bow presentation. This follows the enemy archer/target-switching validation path.
