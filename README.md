...
2. **Interaction & Combat**
   ...
   - **Sprint Attack System**: Dedicated high-damage attack triggered during sprint + movement input + weapon equipped + grounded. Uses independent `SprintAttackMontage` (1.8x damage multiplier, 25 stamina cost). Supports both locked and unlocked states, always attacks toward movement direction. Attack priority: `bIsBlocking` guard → Sprint Attack (highest) → `CanAttack()` → Normal/Combo Attack. Does not chain into combo system (no ComboWindow). Reuses `OnAttackMontageEnded` callback. Replaces old lock-on free-run attack (which was just normal combo first segment). Stops sprinting after attack. Implements direction fallback (`GetActorForwardVector()` when input drops to zero), combo state cleanup (`ResetCombo()` before damage multiplier), and stamina overdraft support (no `CheckStamina` gate). Root motion recommended for displacement.
    - **Data-Driven Combo System**: A multi-segment light attack combo chain configured via `UComboDataAsset` on the player character. The input window is driven by `UAnimNotifyState_ComboWindow` in the animation montage. If the attack key is pressed inside the active window, the input is buffered, and the next segment triggers automatically on montage end with scaled stamina costs and damage multipliers (e.g. 1.0x → 1.2x → 1.5x). Any interruption (getting hit, death, exhaustion, dodging) immediately resets the combo chain and multiplier. Damage multipliers are applied prior to shield blocking, correctly scaling the mitigation math.
...
2. **交互与战斗系统**
   ...
   - **冲刺攻击系统**：冲刺状态下按攻击键触发的高伤害独立攻击（冲刺 + 移动输入 + 已装备武器 + 地面）。使用独立 `SprintAttackMontage`（1.8x 伤害倍率，25 体力消耗）。锁定和非锁定都支持，统一朝移动方向攻击。攻击优先级：`bIsBlocking` 守卫 → 冲刺攻击（最高）→ `CanAttack()` → 普通/连招攻击。不接入连招系统（无 ComboWindow）。复用 `OnAttackMontageEnded` 回调。替代原有的锁定冲刺 free-run 攻击（后者只是普通连招第一段）。攻击后停止冲刺。实现了方向 fallback（输入归零时用 `GetActorForwardVector()`）、连招状态清理（伤害倍率设置前调用 `ResetCombo()`）、体力透支支持（无 `CheckStamina` 门卫）。建议使用根运动驱动位移。
    - **数据驱动连招系统**：在玩家角色上配置的多段轻攻击连招链（通过 `UComboDataAsset` 定义）。连招输入有效窗口由动画蒙太奇中的 `UAnimNotifyState_ComboWindow` 标记。在窗口内按下攻击键时，输入会被缓存并在当前动作播放完毕后自动衔接下一段，每段具有独立的体力消耗和伤害倍率（如 1.0x → 1.2x → 1.5x）。受击、死亡、精疲力竭和翻滚等任何中断都会立即重置连招和伤害倍率。伤害倍率会在盾牌格挡判定前应用，使格挡消耗与减伤公式正确基于加权后的实际伤害进行计算。
...
| `EES_Parried` | 被弹反进入大硬直（默认 1.5s，0.5x 播放速率），锁定移动 |
...
flowchart TD
    A[Tick] --> B{状态守卫}
    B -->|Dead / Stunned / Attacking / Parried| C[return - 跳过本帧]
