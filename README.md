...
For detailed implementation architecture, see [ARCHITECTURE.md](ARCHITECTURE.md).
...
2. **Interaction & Combat**
   ...
   - **Enemy AI**: Full `EEnemyState` FSM with perception, facing verification before attack, attack cooldown system, and 2D BlendSpace-driven locomotion. `EES_Combating` owns a private local HFSM (`EEnemyCombatSubState`: `None`, `Orienting`, `AttackReadyPressing`, `CoordinatedWaiting`, `CooldownSpacing`) to explicitly manage combat sub-behaviors, reducing `OnCombating()` complexity without rewriting the outer state machine. During attack cooldown, enemies actively manage spacing — retreating when too close, strafing laterally within the preferred ring, and pressing forward when at the edge. Attack-ready pursuit uses dynamic `SetGoalActor` tracking to continuously follow the moving player, while cooldown spacing uses static point MoveTo with randomized intervals. See [State Machine Flow](#state-machine-flow) below.
   ...
   - **Enemy Attack Coordination System**: Prevents multiple enemies from attacking simultaneously, creating a strategic "take turns" combat feel. Before attacking, enemies check if nearby allies (within `AttackCoordinationRange`, default 800cm) **chasing the same target** are in `EES_Attacking` state. If allies are attacking, the enemy enters `CoordinatedWaiting` substate and uses `AttackCoordinationBuffer` as the suggested wait time (clamped to `MaxAttackCoordinationWait`, default 3s). Uses traversal-based detection (`GetAllActorsOfClass`) suitable for small-scale demos (< 50 enemies). Coordination checks occur at two entry points: `OnCombating()` Tick (when `!bAttackOnCooldown`) and `OnAttackCooldownEnd()` callback (prevents small-window simultaneous attacks). Coordinated waiting reuses the CD path, automatically triggering `HandleCooldownPositioning` spacing behavior. Debug visualization shows `CombatMove: WaitAlly` in Cyan. Parameters: `AttackCoordinationRange` (800cm), `AttackCoordinationBuffer` (0.5s, 0.3=tight/0.5=natural/0.8=loose), `MaxAttackCoordinationWait` (3s, semantically distinct from `MaxAttackInterval`).
...
1. **核心玩法框架**
   - **角色层 (Character)**：自定义 `AMyCharacter` 类，实现了复杂的运动学逻辑与战斗状态机。通过私有恢复 helper（`RecoverActionStateAfterMontage`、`CleanupInterruptedAttack` 等）收敛重复的蒙太奇结束恢复逻辑，降低后续新动作接入成本。
...
2. **交互与战斗系统**
   ...
   - **敌人 AI**：基于 `EEnemyState` 状态机，支持完整的战斗流程：感知追击 → 面朝校验 → 攻击 → 冷却等待 → 硬直恢复。`EES_Combating` 状态内部使用私有局部 HFSM (`EEnemyCombatSubState`: `None`、`Orienting`、`AttackReadyPressing`、`CoordinatedWaiting`、`CooldownSpacing`)，显式化战斗子行为，降低 `OnCombating()` 继续膨胀风险，无需重写外层状态机。巡逻阶段使用平滑旋转张望,追击阶段使用 2D BlendSpace（Speed × Direction）驱动移动动画。攻击冷却从攻击开始计算，让追击时间重叠冷却，体感更紧凑。冷却期间敌人主动管理距离——过近时后撤，合适距离时侧移绕位，边缘时前压逼近。攻击就绪时改用动态 `SetGoalActor` 追踪持续逼近移动中的玩家，冷却期拉扯则使用静态点位 MoveTo + 随机间隔节流。
   ...
   - **敌人攻击协调系统**：防止多个敌人同时攻击，实现策略性的"轮流攻击"战斗体验。敌人攻击前检查附近队友（`AttackCoordinationRange` 范围内，默认 800cm）**追击同一目标**时是否处于 `EES_Attacking` 状态。如果有队友正在攻击，则进入 `CoordinatedWaiting` 子状态，使用 `AttackCoordinationBuffer` 作为建议等待时间（截断到 `MaxAttackCoordinationWait`，默认 3 秒）。使用遍历检测（`GetAllActorsOfClass`），适合小规模 demo（< 50 个敌人）。协调检查发生在两个入口：`OnCombating()` Tick（当 `!bAttackOnCooldown` 时）和 `OnAttackCooldownEnd()` 回调（防止小窗口同时攻击）。协调等待复用 CD 路径，自动触发 `HandleCooldownPositioning` 拉扯行为。调试可视化显示青色 `CombatMove: WaitAlly` 文字。参数：`AttackCoordinationRange`（800cm）、`AttackCoordinationBuffer`（0.5s，0.3=紧凑/0.5=自然/0.8=宽松）、`MaxAttackCoordinationWait`（3s，与 `MaxAttackInterval` 语义不同）。
