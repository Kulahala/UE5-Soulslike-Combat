...
- **Data-driven attack setup**: `UAttackConfigDataAsset` owns light combo data, special attacks, and charged attack configuration so combat tuning can move out of character code.
...
| Combat Data | Combo segments, special attacks, charged attack config, damage and poise multipliers |
...
- **数据驱动攻击配置**：`UAttackConfigDataAsset` 统一管理轻攻击连招、特殊攻击和蓄力攻击配置，让战斗调参从角色代码中剥离。
...
| 战斗数据 | 连招段、特殊攻击、蓄力攻击配置、伤害倍率、韧性倍率 |
